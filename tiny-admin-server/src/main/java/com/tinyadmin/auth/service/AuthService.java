...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request, HttpServletRequest httpServletRequest) {
        ...
        AuthSession session = AuthSession.builder()
                ...
                .build();
        sessionStore.save(session, Duration.ofDays(7));
        ...
        auditService.upsertOnlineUser(OnlineUserEntity.builder()
                ...
                .expiresAt(LocalDateTime.ofInstant(now.plus(Duration.ofDays(7)), ZoneOffset.UTC))
                .build());
    ...
    public void updatePassword(String oldPassword, String newPassword) {
        AuthSession session = SecurityUtils.currentPrincipal().session();
        UserEntity user = systemLookupService.requireUserById(session.getUserId());
        if (!matchesStoredPassword(user.getPassword(), oldPassword)) {
    ...
    private boolean verifyPassword(UserEntity user, String rawPassword) {
        ...
        if (matchesStoredPassword(storedPassword, rawPassword)) {
            return true;
        }
    ...
    private boolean matchesStoredPassword(String storedPassword, String rawPassword) {
        return storedPassword != null
                && (passwordEncoder.matches(rawPassword, storedPassword) || storedPassword.equals(rawPassword));
    }
